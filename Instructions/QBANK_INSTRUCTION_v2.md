...
### ЧЕКПОИНТ 4: Остальные вопросы + финализация (15-20 мин)
```
Задача: Оформить оставшиеся вопросы, валидация
Результат: frontend/data/qbank/{book}_ch{chapter}_questions.json (полный)
Коммит: "CHECKPOINT 4: Complete {chapter} - {N} questions total"
```
...
### ЧЕКПОИНТ 4: Остальные вопросы

**Вход:** Частичный JSON + `{chapter}_split.json`  
**Выход:** Полный `{book}_ch{chapter}_questions.json`
...
```bash
git add frontend/data/qbank/{book}_ch{chapter}_questions.json
git commit -m "CHECKPOINT 4: Complete {chapter} - {N} questions total"
git push
```
