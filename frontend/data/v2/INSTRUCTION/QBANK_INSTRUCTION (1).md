...
**НАРУШЕНИЕ ЛЮБОГО ИЗ ЭТИХ ПРАВИЛ = БРАК ВСЕЙ РАБОТЫ**

1. **ТОЛЬКО qbank.pdf** — единственный источник вопросов. НИКАКИХ других файлов.
2. **НЕ выдумывать** — даже если вопрос кажется логичным и правильным
3. **НЕ использовать notes.txt** — это для глоссария, НЕ для тестов
4. **НЕ пропускать вопросы** — если таблица сложная, всё равно включи её
5. **НЕ заменять вопросы** — нельзя заменить сложный вопрос на "похожий"
6. **original_question_number** — ДОЛЖЕН точно совпадать с номером Q.XXXX в PDF

### Что делать если не можешь распарсить вопрос:
```
СТОП! Не пропускай и не заменяй.
Напиши пользователю:
"Вопрос Q.XXXX содержит сложную таблицу/формулу, которую не могу корректно извлечь.
Пожалуйста, помоги с форматированием или пропусти этот вопрос явно."
```

### Проверка источника:

Каждый вопрос в JSON должен иметь `original_question_number` который:
- Существует в qbank.pdf
- Находится в диапазоне `question_range` из meta.json для данного модуля
- Module 3: question_range = 76-111 (вопросы Q.76 - Q.111, всего 36 штук)

---

## ✅ ОБЯЗАТЕЛЬНЫЕ ПРОВЕРКИ ПЕРЕД КОММИТОМ

**ВАЖНО:** Эти проверки критически важны и часто пропускаются!

### 1. Проверь ПОСЛЕДНЮЮ страницу PDF

**Проблема:** Последний вопрос в PDF часто пропускается при создании JSON.

**Обязательные шаги:**
1. Открой **последнюю** страницу `qbank.pdf`
2. Найди последний вопрос (номер Q.XXXX)
3. Убедись что этот вопрос **ВКЛЮЧЁН** в JSON
4. Сверь общее количество вопросов:
   - Подсчитай вопросы в PDF (от первого Q.XXXX до последнего Q.YYYY)
   - Проверь что `total_questions` в JSON совпадает

**Пример:**
```
PDF: Q.301 (стр. 1) до Q.4022 (стр. 15) = 16 вопросов
JSON: total_questions должно быть = 16
```

### 2. Проверь диапазон original_question_number

Валидатор показывает диапазон:
```
ℹ️ original_question_number диапазон: 301 - 4022 (16 вопросов)
   Проверь что это совпадает с question_range в meta.json!
```

**Действия:**
- Убедись что первый вопрос совпадает с началом `question_range` в meta.json
- Убедись что последний вопрос совпадает с концом `question_range` в meta.json
- Если не совпадает — проверь PDF ещё раз!

### 3. Итоговый чеклист

- [ ] Открыл последнюю страницу PDF
- [ ] Последний вопрос добавлен в JSON
- [ ] `total_questions` совпадает с реальным количеством вопросов в PDF
- [ ] Диапазон `original_question_number` совпадает с `question_range` в meta.json
- [ ] Валидатор показывает 100% ✓
...
- ❌ `has_table=true` без `table_data` — **ДОБАВЬ ТАБЛИЦУ ИЗ PDF!**
- ❌ `original_question_number` должен быть заполнен и соответствовать PDF
